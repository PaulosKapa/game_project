...
onready var ai = get_parent().get_parent().get_parent()
...
func _ready():
	ai.connect("shoot", self, "_on_KinematicBody_shoot")
	ai.connect("stop_shoot", self, "_on_KinematicBody_stop_shoot")
...
func fire():
	if can_fire == true and magazines.size()>0:
		while magazines[0]>0:
...
func reload():
	if ammo < 1 and magazines.size()>0: #reloading with 0 bullets
		...
		can_fire = true
		reload_value = 0
	elif ammo<1 and magazines.size()<1:
...
func _on_KinematicBody_stop_shoot():
	shoot = false

func _on_KinematicBody_shoot():
	shoot = true

func _on_RayCast_pick():
	if ray.getVar2() == size_of_gun:
		magazines.insert(0,ray.getVar())
